...
class CityMapPoint(val state: String, val city: String, val latitude: Double, val longitude: Double) extends Ordered[CityMapPoint]
{
    ...
    def distance_(x: Double) = {dist = x}

    def setDistance(x: Double) = {dist = x}
    ...
    def addNeighbor(city: CityMapPoint): Unit = {neighborCities = city :: neighborCities}

    def setNeighbors(hood: List[CityMapPoint]) = {neighborCities = hood}

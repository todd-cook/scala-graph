...
class GraphBuildingTest extends FlatSpec {
  ...
  "BuildGraph  " should " build a graph " in {
    ...
    myGraph.addNode(new Node(LatLon(35.25, -119.3), "Bakersfield"))
    myGraph.addNode(new Node(LatLon(34.51, -116.47), "Barstow"))
    myGraph.addNode(new Node(LatLon(33.37, -114.43), "Blythe"))
    myGraph.addNode(new Node(LatLon(34.12, -118.21), "Burbank"))
    myGraph.addNode(new Node(LatLon(39.48, -121.51), "Chico"))
    myGraph.addNode(new Node(LatLon(37.58, -121.59), "Concord"))
    myGraph.addNode(new Node(LatLon(34.5, -117.52), "Covina"))
  ...
  def buildNeighbors(graph: Graph, distanceBetween: Double): Graph = {
    ...
    graph.getNodes.foreach(n => {
      ...
      val verts = vertices.filterNot(a => a == n.vertex) //.vertex.x && a.y == n.vertex.y)
      verts.foreach(b => {
        var dist = 0D
  ...
  "Test 2 " should "check maximum distance between fillups LA to NY" in {
    ...
    assert(scala.math.round(route.totalDistance) == 2644)
  }
  ...
  "Test 3" should "check distance LA to NY" in {
    ...
    println("Total number of paths considered: " + astar.getNumberOfPaths)
    println("number of getMinDistances: " + astar.getMinDistancesSize)
    assert(scala.math.round(route.totalDistance) == 2645)
  }
}
